...
## Data
Six-panel comics, one folder per comic:
```
comics/
  comic1/
    p1.png ... p6.png
  comic2/
    ...
```
Point any script at your comics with `--comics-root` or set `GAZE_COMICS_ROOT` once in your environment. Panels are stitched into a single horizontal strip at load time.
...
If your discovery data is the raw COMICS corpus (`<comic_id>/<page>_<panel>.jpg`), add `--use-raw` and strips are sampled as random consecutive 6-panel windows (seeded, reproducible):
```
python 01_discover_gaze_heads.py --use-raw --comics-root /path/to/raw_panel_images --n-samples 500
```
This is the paper protocol: discover on COMICS, evaluate steering on a disjoint held-out set.
...
Steering adds a single pre-softmax bias on the gaze heads' attention: boost the target panel's image tokens, suppress the rest. Nothing is retrained — fewer than 9% of the heads carry the lever.
